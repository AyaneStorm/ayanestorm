...
bool LLGLSLShader::createShader()
{
    ...
    if (!mUsingBinaryProgram)
    {
        ...
        for (; fileIter != mShaderFiles.end(); fileIter++)
        {
            ...
            const S32 texture_index_channels = (*fileIter).first == "deferred/exactOITCaptureF.glsl" ?
                -1 : mFeatures.mIndexedTextureChannels;
            GLuint shaderhandle = LLShaderMgr::instance()->loadShaderFile(
                (*fileIter).first, mShaderLevel, (*fileIter).second, &mDefines, texture_index_channels);
            // </AS:Chanayane>
