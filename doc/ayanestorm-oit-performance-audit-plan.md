...
| 2026-09-03 | **AVBOIT hair flicker regression** | found, not investigated. Zooming in on part of the avatar's hair shows frame-to-frame flicker in AVBOIT; confirmed absent on a pre-session viewer build, so this is a regression from this session's AVBOIT work (A1-A7), not pre-existing. Debug mode 14 (sampled front transmittance, bypasses final composite math) flickers at the same spot, pointing at the transmittance-volume/sampling path (A1, A2, A6 candidates) rather than A7 (which only touched the final opaque-colour read, downstream of what mode 14 samples). Not investigated; logged as a TODO per user request. Details in `ayanestorm-oit-avboit-hair-flicker-regression-todo.md` |
