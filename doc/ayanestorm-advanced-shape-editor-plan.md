...
- Undo stores complete pre-change override-map snapshots. Slider drags are coalesced into one undo action, and Reset Joint/Reset All are each one undoable action. `ASBoneDeformerUndoLevels` is configurable under AyaneStorm Preferences from 1 to 99 and defaults to 99.
- The floater warns users to remove other deformers because UUID-priority conflicts can hide this session's overrides. Its T-pose button temporarily selects `PS_T_Pose` through the existing Firestorm Pose Stand and restores the prior Pose Stand selection/state when toggled off or when the deformer closes.
